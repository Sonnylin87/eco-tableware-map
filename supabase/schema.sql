...
create table if not exists reviews (
  ...
  offers_delivery     text not null default 'unknown' check (offers_delivery in ('yes', 'no', 'unknown')),
  notes               text,
  created_at          timestamptz not null default now()
);

-- 舊專案（第一版就已經跑過這份 schema）不會自動長出新欄位，
-- 這幾行補齊 reusable_bowl / reusable_plate / provides_straw / offers_delivery。
...
alter table reviews add column if not exists offers_delivery text not null default 'unknown' check (offers_delivery in ('yes', 'no', 'unknown'));
