...
一個群眾協作的地圖網站：任何拿到連結的人都能在地圖上標記餐廳，並用「表格式」評論勾選這家餐廳是否提供環保餐具、環保杯、環保碗、環保盤、吸管。不需要登入。所有勾選項目定義在 `js/aggregate.js` 的 `CHECKLIST_FIELDS`，要再新增項目只要改那裡（同時記得在 `supabase/schema.sql` 加對應欄位）。
