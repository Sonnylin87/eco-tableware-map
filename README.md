...
一個群眾協作的地圖網站：任何拿到連結的人都能在地圖上標記餐廳，並用「表格式」評論勾選這家餐廳是否提供環保餐具、環保杯、環保碗、環保盤、是否可外送。不需要登入。所有勾選項目定義在 `js/aggregate.js` 的 `CHECKLIST_FIELDS`，要再新增項目只要改那裡（同時記得在 `supabase/schema.sql` 加對應欄位）。「可外送」這項比較特別：多數決結果是「有提供」時，地圖標記會變成黃色（其他項目不影響標記顏色），邏輯在 `js/map.js` 的 `markerColorForAggregate`。

- 地圖：OpenStreetMap + Leaflet.js
- 定位：瀏覽器內建 Geolocation API（原本還有 Nominatim 地址查詢，2026-09-11 因為查不太到台灣門牌而拔掉，見下方「已知限制」）
...
2. 按「新增店家」→ 兩種方式都測一次：點地圖上任一位置 / 使用我目前的位置 → 填店名 + 勾選項目 → 送出 → 畫面立刻出現新的彩色標記。
...
| `js/aggregate.js` | 統計每家餐廳的評論、決定各項目的多數決結論（標記本身統一顏色，不受此影響） |
| `js/geolocate.js` | 「定位我」功能，讀取瀏覽器目前位置 |
| `js/map.js` | Leaflet 地圖初始化、標記渲染、點地圖/用目前位置新增店家 |
...
前端表單有隱藏欄位 honeypot，但那只防得住乖乖填表單的陽春機器人——anon key 本來就公開，任何人都能繞過網站直接打 API。`supabase/schema.sql` 第 7 節補了兩層防線，貼進 SQL Editor 執行即可生效：

- **欄位長度／座標範圍檢查**：店名/地址/備註/回報留言都有長度上限，座標限制在台灣本島＋外島的合理範圍內。用 `not valid` 加的，不會去檢查、也不會擋掉既有的舊資料，只管接下來新增/修改的部分。
- **依來源 IP 的頻率限制**：同一個 IP 短時間內新增太多筆餐廳/評論/回報會被擋掉（門檻很寬鬆，正常手動操作不會碰到，只用來擋腳本狂發）。這不是無敵防線——IP 標頭理論上可以被偽造，同一個 wifi/IP 底下很多人同時使用也會共用同一個額度——但能真正擋掉批量洗版的腳本，而不只是防手動填表單的人。門檻數字在 `schema.sql` 的 `check_rate_limit('xxx', 次數, 分鐘數)` 那幾行，之後覺得太嚴或太鬆都可以直接改數字重新執行。

## 之後可以考慮加強的地方（目前故意先不做，保持簡單）

- 一般使用者的帳號登入、查看自己送出過的回報/評論紀錄
- 地址查詢：2026-09-11 因為 Nominatim 查不太到台灣門牌號碼而拔掉了，之後如果想找替代方案再重新評估（不要重試 Nominatim 或已經被拒絕的 TGOS，見專案筆記）
