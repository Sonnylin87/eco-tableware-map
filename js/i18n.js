...
const translations = {
  zh: {
    ...
    help_title: '❓ 這是什麼網站？',
    help_intro:
      '這是任何人都能一起維護的地圖：標出提供環保餐具（環保杯／碗／盤／餐具）、以及是否可外送的餐廳，幫大家減少一次性用品。不需要登入、不需要註冊，人人都能標記、都能留備註。',
    help_view_heading: '🔍 查看一家店',
    help_view_body:
      '點地圖上的圓點，會彈出這家店目前的統計（各項目的結論、大家留的備註），也可以在這裡按「🧭 導航」開 Google 地圖規劃路線，或「回報問題」。多數決結果是「可外送」的店家，圓點會顯示黃色，其他店家都是綠色。',
    help_locate_heading: '📍 定位我',
    help_locate_body: '點右上角「定位我」，會用手機/瀏覽器的定位功能，在地圖上用藍色圓點標出你目前的位置。',
    help_add_heading: '➕ 新增一家店',
    help_add_body:
      '點右上角「新增店家」，可以選擇：在地圖上點選位置，或使用目前所在位置。新增時會順便請你勾選環保餐具/杯/碗/盤、以及是否可外送的情形，地址欄位選填。',
    help_review_heading: '✍️ 填寫評論',
    help_review_body:
      '對一家店的環保餐具/杯/碗/盤、可外送情形，勾選「有提供／沒有提供／不確定」，可以留一句 50 字內的備註（例如「需要主動詢問店員」）——備註會公開顯示在地圖上給其他人看。同一家店可以有很多人各自留備註。',
  ...
  en: {
    ...
    help_intro:
      'A crowdsourced map: it marks restaurants that offer reusable tableware (cups/bowls/plates/utensils) and whether they offer delivery, to help everyone cut down on single-use items. No login or sign-up needed — anyone can add a place or leave a note.',
    help_view_heading: '🔍 View a place',
    help_view_body:
      'Tap a dot on the map to see its current stats (the verdict for each item, and notes people left). From there you can also tap "🧭 Directions" to open Google Maps, or "Report an issue". Places where the majority verdict for delivery is "yes" show a yellow dot; everyone else is green.',
    ...
    help_add_body:
      'Tap "Add a place" in the top bar and choose: pick a spot on the map, or use your current location. You\'ll also be asked to mark the reusable tableware/cup/bowl/plate and delivery situation; the address field is optional.',
    help_review_heading: '✍️ Leave a review',
    help_review_body:
      'For a place\'s reusable tableware/cup/bowl/plate and delivery situation, choose "Yes / No / Not sure" for each item. You can also leave a note up to 50 characters (e.g. "Need to ask staff") — notes are shown publicly on the map. Many people can leave their own notes for the same place.',
    ...
    field_offers_delivery: 'delivery',
